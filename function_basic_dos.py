...
def print_and_return(thelist):
    print(thelist[0])
    return thelist[1]
print(print_and_return([5, 9, 6, 7]))
...
def first_plus_length(thelist):
    ...
    return sum
print(first_plus_length([5,1,1,1]))
...
def values_greater_than_second(thelist):
    newlist = []
    for x in range(0, len(thelist)):
        if (thelist[1] < thelist[x]):
            newlist.append(thelist[x])
    if (len(newlist) < 2):
        return False
    else:
        return newlist
print(values_greater_than_second([2,3,5,6,7,7]))
